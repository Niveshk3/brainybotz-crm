...
from .forms import (
    ...
    MultipleMediaForm, AnnouncementForm, ProfileForm, OTPRequestForm, OTPVerifyForm, AchievementForm, CertificateForm
)
from .models import (
...
def portfolio(request):
    ...
    attendance_total = student.attendances.count()
    attendance_present = student.attendances.filter(status__in=["PRESENT", "LATE"]).count()
    attendance_pct = round(attendance_present * 100 / attendance_total, 1) if attendance_total else 0
...
def achievement_add(request):
    ...
    if r == "STUDENT":
        ...
        if not student_id:
            ...
            return redirect("student_list")
        student = get_object_or_404(Student, pk=student_id)
        if r == "TRAINER" and (not student.batch_id or student.batch.trainer_id != request.user.trainer.id):
            return redirect("dashboard")
        if r not in ["ADMIN", "TRAINER"]:
            return redirect("dashboard")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            Notification.objects.create(
                ...
                message=f"'{achievement.title}' was added to your achievement portfolio.",
                notification_type="ACHIEVEMENT",
                action_url=f"/portfolio/?student={student.id}",
            )
            messages.success(request, "Achievement added to the portfolio.")
            return redirect(f"/portfolio/?student={student.id}")
    else:
        form = AchievementForm()

    return render(request, "form.html", {
        "form": form,
        "title": f"Add Achievement — {student.full_name}",
    })
...
def certificate_add(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            Notification.objects.create(
                ...
                message=f"Your certificate '{certificate.title}' has been added to your portfolio.",
    ...
    return render(request, "form.html", {
        "form": form,
        "title": f"Add Certificate — {student.full_name}",
    })
...
# Phase 7 analytics/reporting endpoints
from .analytics import analytics, report_csv
